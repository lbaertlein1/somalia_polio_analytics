  ...
  tagList(
    ...
    div(
      ...
      tags$p(
        ...
        'Each health area is coordinated by one SIA site, ideally covering ',
        tags$strong('~2,000 children'), ' with ', tags$strong('~5 vaccination teams. '),
...
healthAreaControlsServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$brush_value_label <- renderUI({
      val <- input$brush_m_ui
      if (is.null(val)) return(NULL)
      tags$span(paste0(val, ' m'))
    })
