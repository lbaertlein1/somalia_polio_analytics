...
healthAreaTabServer <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # ── Pending restore ────────────────────────────────────────────────────────
    # Stores a snapshot waiting for the grid to be ready.
    # Set by restore observer; consumed by observeEvent(selected_scene()).
    ...
    district_base <- reactive({
      ...
      district_sf <- district_sf |>
        dplyr::summarise(admin_id = dplyr::first(admin_id), district_name = dplyr::first(district_name),
      ...
      list(district_sf = district_sf, max_dim_m = max_dim_m,
    ...
    observeEvent(controls$show_pop_raster(), {
      ...
      if (isTRUE(controls$show_pop_raster()) && is.null(rv$pop_overlay_sf) && !is.null(rv$district_sf)) {
        rv$pop_overlay_sf <- tryCatch(
          ...
          error = function(e) { cat('population overlay error:', e$message, '\n'); NULL }
        )
      }
      geojson <- if (!is.null(rv$pop_overlay_sf)) as_geojson_text(rv$pop_overlay_sf) else NULL
      send_paint_message("paint_toggle_population", list(show = controls$show_pop_raster(), geojson = geojson))
    }, ignoreInit = TRUE)
    observeEvent(controls$show_friction_raster(), {
      ...
      if (isTRUE(controls$show_friction_raster()) && is.null(rv$friction_overlay_sf) &&
        ...
        rv$friction_overlay_sf <- tryCatch(
          make_friction_overlay_sf(district_sf = rv$district_sf, friction_rast = terra::rast(rv$friction_path)),
          error = function(e) { cat('friction overlay error:', e$message, '\n'); NULL }
        )
      }
      geojson <- if (!is.null(rv$friction_overlay_sf)) as_geojson_text(rv$friction_overlay_sf) else NULL
      send_paint_message("paint_toggle_friction", list(show = controls$show_friction_raster(), geojson = geojson))
    }, ignoreInit = TRUE)
    ...
    send_current_scene <- function() {
      ...
      cat("[health_area] facility_pts:", length(facility_pts),
          "| landmark_pts:", length(landmark_pts), "\n")
    ...
    # ── Apply a snapshot once grid is ready ───────────────────────────────────
    .apply_restore <- function(snap) {
      restore_just_applied(TRUE)
      if (!is.null(snap$dfa_names)) rv$dfa_names <- snap$dfa_names
      
      if (!is.null(snap$current_assignments)) {
        ca <- snap$current_assignments
        # Convert named list → ordered character vector by cell_id
        if (!is.character(ca) || is.list(ca)) {
      ...
      cat("[health_area] restore applied\n")
    }
    
    # ── Grid becomes ready: apply pending restore OR use fresh assignments ─────
    observeEvent(selected_scene(), {
      sc <- selected_scene()
      
      # ── Check restore_just_applied FIRST before touching any rv state ─────────
      # selected_scene can fire a second time after restore (e.g. when
      # facility_seed_sf changes). In that case keep all restored state and
      # only update non-assignment metadata.
      ...
      # ── Normal path — update all rv state ─────────────────────────────────────
    ...
    # ── Restore observer: queue if grid not ready, apply immediately if it is ─
    observeEvent(restore_r(), {
      snap <- restore_r()
      if (is.null(snap)) return()
      if (!is.null(snap$current_assignments) && length(snap$current_assignments) > 0) {
        if (!is.null(rv$grid_sf)) {
          .apply_restore(snap)
        } else {
          pending_restore(snap)
          cat("[health_area] restore queued — grid not ready yet\n")
        }
      }
    ...
    observeEvent(map_mod$assignments(), {
      ...
      # ── Capture: sync JS → R + rebuild boundary, no DB write ─────────────────
      ...
      # ── Save: build polygons, write to DB, checkpoint session ─────────────────
    ...
    observeEvent(active_dfa_rv(), {
      ...
      # Capture current JS assignments into R without saving to DB
    ...
    list(
      ...
      restore_from_snapshot = function(snap) {
        if (!is.null(rv$grid_sf)) {
          ...
          pending_restore(snap)
          cat("[health_area] restore queued via restore_from_snapshot\n")
        }
      }
    )
  })
